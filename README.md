...
1. We start by defining the `slug` parameter and giving it a random value of 10 alpha-numeric characters at the start of every scenario iteration. In order to do that we click on **Advanced Settings** at the bottom of the page, scroll to the **Parameter Defaults** section and define the value for the `slug` parameter to be `${__random_chars}` - using the `${}` syntax here works the same as before, i.e. it instructs the Loadmill interpreter to use a value stored in a parameter. The only difference is that, this time, we are using a **built-in  parameter** that resolves a different random value each time it is evaluates.
